	...
	if (FSL_Rst | rst)
		...
		RDADDR: 
			begin
				...
				if (fifo_counter == 1 & pixel_af_wr_en) begin
		...
		FTDATA: 
			begin
				if(XIL_NPI_AddrAck & pixel_af_wr_en) begin
					state <= WAITEMP;
					counter <= 3'd4;
				end
				pixel_rdf_valid <= 0;
			...
			begin
				if((~XIL_NPI_RdFIFO_Empty) & XIL_NPI_InitDone) begin
					if(latency_counter == 0)
						begin
							//temp_npi_data[((pixel_ind[2:0]+2)*32-1):(pixel_ind[2:0]*32)] <= XIL_NPI_RdFIFO_Data;
							//if (pixel_af_wr_en)
							rdf_dout <= XIL_NPI_RdFIFO_Data;
							pixel_rdf_valid <= 1;
							if (pixel_ind == 20'd786430) //1024*768-2
								pixel_ind <= 0;
							else
								pixel_ind <= pixel_ind + 2;
							if (pixel_ind[2:0] == 3'b110)
								state <= RDADDR;
						end
					else
						begin
...
module fifo_generator_v9_1 (
  rd_en, rst, empty, wr_en, rd_clk, full, wr_clk, dout, din
